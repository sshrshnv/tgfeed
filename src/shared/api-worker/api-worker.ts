...
import { createPromise, setDelay } from '~/shared/utils'
import type { DB, DBSchema, DBWorker } from '~/shared/db-worker'
...
self.onmessage = (ev: MessageEvent<APIWorkerMessage>) => {
  resolveDbWorkerPromise(wrap(ev.data.dbPort) as DBWorker)
  expose(apiWorker, ev.data.mainPort)
}
